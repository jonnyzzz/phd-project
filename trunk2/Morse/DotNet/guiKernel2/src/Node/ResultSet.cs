...
namespace EugenePetrenko.Gui2.Kernell2.Node
{
  ...
  public class ResultSet : IEnumerable
  {
    ...
    {
      get { return (IResult) results[index]; }
    }
    ...
    public KernelNode[] ToNodes
    {
      get
      {
        ...
        if (results.Length > 0)
        {
          ...
          {
            nodes.Add(new KernelNode(FromResult(result)));
          }
        }
        else
        {
          nodes.Add(new KernelNode(Empty()));
        }
    ...
    public bool Match(string resultInterface, string metadataInterface)
    {
      ...
      foreach (IResult result in results)
      {
        ...
        if (
          !
          (typeFinder.ImplementsType(result, resultInterface) &&
           typeFinder.ImplementsType(result.GetMetadata(), metadataInterface)))
        {
          return false;
        }
      }
    ...
    {
      return string.Format("ResultSet [ length = {0} ]", results.Count);
    }
