  ...
  public class CommandLineParser
  {
    ...
    public CommandLineParser(string[] commandLine)
    {
      for (int i = 0; i < commandLine.Length; i++)
      {
        ...
        if (!param.StartsWith("/"))
          throw new CommandLineParseException("Unexpected key. All keys should starts from '/'");
        ...
        if (data.Length > 2)
          throw new CommandLineParseException("Unexpected key style. Was :" + commandLine[i] +
                                              " but expected /key[=value]");
