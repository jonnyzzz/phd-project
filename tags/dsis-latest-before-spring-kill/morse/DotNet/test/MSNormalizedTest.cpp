...
#include "../cellImageBuilders/MSNormedCreationProcess.h"
...
#include "../AdaptiveCellImageBuilder/MSNormedProcess.h"
...
MSNormalizedTest::MSNormalizedTest(void) : TestBase("MSNormalized 3D", cout)
{
}

MSNormalizedTest::~MSNormalizedTest(void)
{
}
...
void MSNormalizedTest::Test() {
	...
	SmartProcess proc = new MSNormedCreationProcess(aResult, msFactor, &pinfo);
	proc->start();
	...
	MULTI(
	{
		...
		SmartProcess pb = new MSNormedProcess(func, dfunc, cms->getAt(0), msFactor2, precision, &pinfo);
		pb->start();
		for (int i=0; i<cms->length(); i++) {
