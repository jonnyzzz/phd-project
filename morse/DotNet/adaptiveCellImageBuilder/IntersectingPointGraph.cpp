...
PointGraphAction IntersectingPointGraph::NeedDevideEdge(const double* left, const double* right, const double* precision) {
	if (!graph->intersects(left, 0, dimension) || !graph->intersects(right, 0, dimension)) 
		return PointGraph_NotDevide;

    for (int i=0;i<dimension; i++) {
		if (Abs(*left++ - *right++) > *precision++) return PointGraph_Devide;
    }
	return PointGraph_NotDevide;
}