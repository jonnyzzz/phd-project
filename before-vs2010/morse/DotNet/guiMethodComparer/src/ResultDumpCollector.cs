...
namespace Eugene.Petrenko.Gui2.MethodComparer
{
  ...
  public class ResultDumpCollector
  {
    ...
    public void SaveXMLLog(string file)
    {
      ...
      using (
        Stream stream =
          typeof (ResultDumper).Assembly.GetManifestResourceStream(
            "Eugene.Petrenko.Gui2.MethodComparer.xml.transpose.xsl"))
      {
        transform.Load(new XmlTextReader(new StreamReader(stream)), new XmlUrlResolver(), GetType().Assembly.Evidence);
      }
